...
if __name__ == '__main__':
    ...
    inSoftmax_tbuffer = tbuffer('inSoftmax', n_head/num_chip*batch*batch*word) # attention
    ...
    in3_tbuffer = tbuffer('in3', n_head/num_chip*batch*batch*word) # attention
    ...
    model.addConstr(Config[3] == 0)
    model.addConstr(Config[4] == 1)
    model.addConstr(Config[5] == 1)
    model.addConstr(Config[6] == 1)
    model.addConstr(Config[7] == 1)
    model.addConstr(Config[8] == 2)
    model.addConstr(Config[9] == 2)
    model.addConstr(Config[10] == 3)
    ...
    for i in range(C):
        ...
        t3 = model.addMVar(Nb, vtype=gp.GRB.INTEGER, lb=0)
        
        for j in range(Nc):
            model.addConstr(t1[j] == Cycle[j] @ Ac[j, i])
        
        for j in range(Nb):
            model.addConstr(t3[j] == Ab1[j, i] @ Tileb[j])
        
        model.addConstr(t2[0].tolist()[0] == gp.max_(t1[j].tolist()[0] for j in range(Nc)))
        model.addConstr(Compute_Latency[i] == t2[0] / Freq)
        model.addConstr(DRAM_Latency[i] == (TSb_notile @ t3 + TSb_notile @ Ab2[:, i]) / DRAM_BW)
